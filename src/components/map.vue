...
onMounted(() => {
    ...
    let religiIcon = new L.Icon({
        iconUrl: `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" style="fill: red;" viewBox="0 0 384 512"><!--! Font Awesome Pro 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M215.7 499.2C267 435 384 279.4 384 192C384 86 298 0 192 0S0 86 0 192c0 87.4 117 243 168.3 307.2c12.3 15.3 35.1 15.3 47.4 0zM192 128a64 64 0 1 1 0 128 64 64 0 1 1 0-128z"/></svg>`,
        iconSize: [30, 30],
        iconAnchor: [15, 30],
        popupAnchor: [0, -30],
    });
    ...
    axios.get('https://admin.api.northexpokudus.com/api/destinasi').then((response) => {
        ...
        data.forEach((item) => {
            ...
            } else if (item.jenis == 'Taman Hiburan') {
                L.marker([item.latitude, item.longitude], { icon: budayaIcon }).addTo(map).bindPopup(`<b>${item.nama}</b><br>${item.alamat}<br><a href="${item.link}">Lihat Detail</a>`)
            } else {
